...
export default function ContactInfo({ 
  ...
  return (
    <div className={containerClasses}>
      {/* Email */}
      {profile.email && (
        <div className={layout === 'horizontal' ? 'flex items-center gap-2' : 'text-center'}>
          ...
          <a 
            ...
            className={`${linkClasses} transition-colors text-sm font-medium`}
          >
            {profile.email}
          </a>
        </div>
      ...
      {profile.socialLinks.map((link) => (
        <div key={link.id} className={layout === 'horizontal' ? 'flex items-center gap-2' : 'text-center'}>
          {/* Social Media Icons */}
          ...
          {!['twitter', 'github', 'linkedin', 'google scholar'].includes(link.platform.toLowerCase()) && (
            <svg className="w-4 h-4 text-sage-green" fill="currentColor" viewBox="0 0 20 20">
