...
export default function PublicationList({
  ...
  const baseClasses = theme === 'dark' ? 'bg-gray-800 text-white' : 'bg-white';
    ...
    <div className={`space-y-${compact ? '3' : '4'}`}>
        ...
        <div key={publication.id} className={`card ${baseClasses}`}>
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1">
              <h3
                className={`${compact ? 'text-sm' : 'text-lg'} font-semibold ${textClasses.primary} leading-tight`}
              >
                {publication.url ? (
                  <a
              ...
              <div
                className={`${compact ? 'text-xs' : 'text-sm'} ${textClasses.secondary} mt-1`}
              >
              ...
              <div
                className={`${compact ? 'text-xs' : 'text-sm'} ${textClasses.muted} mt-1`}
              >
                {publication.journal && (
              ...
              {detailed && publication.abstract && (
                <p
                  className={`${compact ? 'text-xs' : 'text-sm'} ${textClasses.secondary} mt-3 leading-relaxed`}
                >
              ...
              {publication.keywords && publication.keywords.length > 0 && (
                <div className="mt-2 flex flex-wrap gap-1">
                  {publication.keywords.slice(0, 5).map((keyword, index) => (
                    <span
                      key={index}
                      className="bg-sage-green text-sage-green inline-flex items-center rounded-full bg-opacity-20 px-2 py-1 text-xs"
                    >
                      {keyword}
                    </span>
                  ))}
            ...
            <div className="flex-shrink-0 text-right">
              {publication.citationCount !== undefined && (
                <div
                  className={`${compact ? 'text-xs' : 'text-sm'} ${textClasses.muted}`}
                >
              ...
              <div className="mt-2 space-y-1">
                {publication.doi && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-sage-green block text-xs hover:underline"
                  >
                    DOI
                  </a>
                )}
      ...
      {!showAll && publications.length > 5 && (
        <div className="pt-4 text-center">
          <button className="text-sage-green font-medium hover:underline">
